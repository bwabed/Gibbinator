...
class NachrichtRepository extends Repository
{
    ...
    public function get_message_for_student_sorted($klassenIDs, $lektionIDs, $fachIds, $max = 100) {
        ...
        if ($klassenPart . $lektionPart . $fachPart != "") {
            ...
            $statement = ConnectionHandler::getConnection()->prepare($query);
            $statement = $this->dynamic_bind_variables($statement, $klassenIDs, $lektionIDs, $fachIds);
            $statement->execute();
            $result = $statement->get_result();
            if (!$result) {
                throw new Exception($statement->error);
    ...
    public function get_message_for_prof_sorted($creatorID, $klassenIDs, $lektionIDs) {
        ...
        $statement = ConnectionHandler::getConnection()->prepare($query);
        $statement->bind_param('i', $creatorID);
        $statement = $this->dynamic_bind_variables($statement, $klassenIDs, $lektionIDs);
        $statement->execute();
        $result = $statement->get_result();
        if (!$result) {
            throw new Exception($statement->error);
