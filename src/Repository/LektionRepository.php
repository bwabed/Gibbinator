...
class LektionRepository extends Repository
{
    protected $tableName = 'lektion';
    protected $datesTable = 'date';
    ...
    public function get_lektionen_by_klassen_ids($klassenIDs) {
        ...
        $this->dynamic_bind_variables($statement, $klassenIDs, null);
        $statement->execute();
    ...
    public function get_date_by_lektionen($lektionDateIDs) {
        ...
        $statement = ConnectionHandler::getConnection()->prepare($query);
        $statement = $this->dynamic_bind_variables($statement, $lektionDateIDs, null);
        $statement->execute();
        $result = $statement->get_result();
        if (!$result) {
            throw new Exception($statement->error);
    ...
    public function get_lektionen_by_faecher($faecherIds) {
        ...
        $statement = ConnectionHandler::getConnection()->prepare($query);
        $statement = $this->dynamic_bind_variables($statement, $faecherIds);
        $statement->execute();
        $result = $statement->get_result();
        if (!$result) {
            throw new Exception($statement->error);
    ...
    function dynamic_bind_variables($stmt, $params, $params2 = null)
    {
        if ($params != null)
        {
