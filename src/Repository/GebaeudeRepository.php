...
class GebaeudeRepository extends Repository
{
    ...
    public function get_rooms_by_ids($roomIds) {
        ...
        $statement = ConnectionHandler::getConnection()->prepare($query);
        $this->dynamic_bind_variables($statement, $roomIds, null, null);
        $statement->execute();
        $result = $statement->get_result();
        if (!$result) {
            throw new Exception($statement->error);
    ...
    private function dynamic_bind_variables($stmt, $params, $params2, $params3)
    {
        if ($params != null)
        {
