    ...
    <div class="mdl-card mdl-card-form mdl-cell--12-col mdl-grid mdl-grid--no-spacing mdl-shadow--2dp" id="user_results">
        ...
        <table class="customTable mdl-cell--12-col mdl-data-table mdl-js-data-table mdl-data-table--selectable mdl-shadow--2dp"
            ...
            foreach ($users as $row) {
                echo '
          <tr data-id="' . htmlentities($row->id) . '">
          <td>
          <form id="form-select-user-' . htmlentities($row->id) . '" action="/admin/edit_user" method="post">
          <a href="#" id="form-select-user-button-' . htmlentities($row->id) . '" class="mdl-navigation__link">
          <i class="material-icons">edit</i>
          </a>
          <input type="hidden" name="user_id" value="' . htmlentities($row->id) . '">
          <script>
          $(document).ready(function() {
            $("#form-select-user-button-' . htmlentities($row->id) . '").click(function(e) {
              $("#form-select-user-' . htmlentities($row->id) . '").submit();
            });
          });
          </script>
          </form>
          ...
          <td>' . htmlentities($row->nachname) . '</td>
          <td>' . htmlentities($row->email) . '</td>
          <td>';
                foreach ($usertypes as $usertype) {
                    if ($usertype->id == $row->user_type) {
                        echo htmlentities($usertype->bezeichnung);
                    }
                }
                echo '</td>
          <td>';
